...
public class HfsPlusFileSystemTest
{
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/test.hfsplus");
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/hard-linked-directories.dmg");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/hard-linked-files.dmg");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/compressed.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    public void testDiskWithIncorrectCompressedFileOnFile() throws Exception
    {
        ...
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/wrong-compressed-flag.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    @Ignore("failing at whereFromAttr")
    @Test
    public void testDiskAttributes() throws Exception
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/attributes.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
            ...
            assertThat(whereFrom, is("bplist"));
            assertThat(quarantine, is("0081;5cc8ba96;Chrome;3792716B-4DE2-4A43-BFA3-516714DD5764"));
        }
        finally
        {
            testFile.delete();
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/compressed-hardlinks.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/large-compressed.dmg");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    @Ignore("test data missing")
    @Test
    public void testDiskWithLzvnCompression() throws Exception
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/hfsplus/rle-compression.dmg");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            HfsPlusFileSystem fs = type.create(device, true);
    ...
    public void testCreate() throws Exception
    {
        ...
        try(FileDevice device = new FileDevice(testFile, "rw"))
        {
            HfsPlusFileSystemType type = fss.getFileSystemType(HfsPlusFileSystemType.ID);
            ...
            assertThat(vh.getMagic(), is((SuperBlock.HFSPLUS_SUPER_MAGIC)));
            assertThat(vh.getBlockSize(), is(4096));
        }
        finally
        {
            testFile.delete();
    ...
    @Ignore("fails when writing catalog")
    @Test
    public void testRead() throws Exception
    {
        ...
        try (FileDevice device = new FileDevice(testFile, "rw"))
        {
            ...
            assertThat("Must be empty", root.iterator().hasNext(), is(false));
            root.addDirectory("test");
            fs.flush();
            fs.close();
            fs = new HfsPlusFileSystemType().create(device, false);
            fs.read();
            assertThat(fs.getVolumeHeader().getFolderCount(), is(1));
            fs.createRootEntry();
            root = fs.getRootEntry().getDirectory();
            assertThat("Must contains one directory", root.iterator().hasNext(), is(true));
        }
        finally
        {
            testFile.delete();
