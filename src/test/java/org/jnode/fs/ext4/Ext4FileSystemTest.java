...
public class Ext4FileSystemTest
{
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-64bit.dd");
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-selinux.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/inline.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/test-special-files.ext4");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    {
        File testFile =FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-mmp.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    public void testReadExt4LargeDirectory() throws Exception
    {
        ...
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-large-directory.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
            ...
            while (iterator.hasNext())
            {
                ...
                if (entry.isFile())
                {
                    assertThat(DataStructureAsserts.getMD5Digest(entry.getFile()), is("b1946ac92492d2347c6235b4d2611184"));
                    childCount++;
                }
            }

            assertThat(childCount, is(65001));
        }
        finally
        {
            testFile.delete();
    ...
    public void testReadExt4LargeDirectoryWithIndex() throws Exception
    {
        ...
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-large-dir-with-index.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-flex-bg.img");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
    ...
    {
        File testFile = FileSystemTestUtils.getTestFile("org/jnode/fs/ext4/ext4-meta-bg.dd");
        try (FileDevice device = new FileDevice(testFile, "r"))
        {
            Ext2FileSystemType type = fss.getFileSystemType(Ext2FileSystemType.ID);
            Ext2FileSystem fs = type.create(device, true);
